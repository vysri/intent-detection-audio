...
import numpy as np
from train_intent_classifier_text import TextIntentClassifier, INTENTS, INTENT_TO_ID

def evaluate_dataset(jsonl_path, model_path):
    """
    Evaluate text-based intent classifier on entire dataset.
    ...
    with open(jsonl_path, "r") as f:
        for line in f:
            ...
            if "sentence_embedding" in sample:
                samples.append(sample)

    if len(samples) == 0:
        print("ERROR: No samples with sentence_embedding found. Did you run add_sentence_embeddings.py?")
        return

    print(f"Loaded {len(samples)} samples\n")

    # Get embedding dimension from first sample
    embedding_dim = len(samples[0]["sentence_embedding"])
    ...
    with torch.no_grad():
        for idx, sample in enumerate(samples):
            ...
            embedding = torch.tensor(sample["sentence_embedding"], dtype=torch.float32).to(device)
    ...
    # Compute metrics
    print("\n" + "="*60)
    print("TEXT-BASED EVALUATION RESULTS")
    print("="*60)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        default="checkpoints/intent_classifier_text.pt",
        help="Path to trained model",
    )

    args = parser.parse_args()

    evaluate_dataset(args.dataset, args.model)
